...
1. [Download sequences](#1_Download_sequences) 
   1. [Download coral protein sequences](#1.1 Download coral protein sequences)
   2. [Download NOX reference sequences](#1.2 Download NOX reference sequences)
   3. [Download transcriptomic sequences and translate to proteins](#1.3 Download transcriptomic sequences and translate to proteins)
2. [Query annotated coral sequences](#2. Query annotated coral sequences)
   1. [Query annotated sequences for NOX proteins](#2.1 Query annotated sequences for NOX proteins)
3. [Identify NOX-like coral protein sequences in unannotated sequences](#3. Identify NOX-like coral protein sequences in unannotated sequences)
   ...
   1. [Create protein family (pfam) profiles using hidden markov models (hmm)  for each NOX-type ](#3.1 Create protein family (pfam) profiles  using hidden markov models (hmm) for each NOX-type )
   2. [Query unannotated coral protein sequences using hmmSearch](#3.2 Query unannotated coral protein sequences using hmmSearch)
   3. [Identify NOX-like coral sequences](#3.3 Identify NOX-like coral sequences)
   4. [Verify identified NOX-like sequences against *nr* database](#3.4 Verify identified NOX-like sequences against *nr* database) 
5. [NOX Domain Analysis](#4. NOX Domain Analysis)
   
   1. [Download NOX-human domains from online pfam database](#4.1 Download NOX-human domains from online pfam database)
   2. [Identify specific domains in NOX-like sequences](#4.2 Identify specific domains in NOX-like sequences)
6. [Create a phylogenetic tree](#5.1 Create a phylogenetic tree)
   
   1. [Align all coral NOX-like sequences](#Align all coral NOX-like sequences)
   
   2. [Create a tree file](#5.2 Create a tree file)
   
   3. [Overall bash script to execute for creating a tree](#5.3 Overall bash script to execute for creating a tree)
   
   4. [Visualize tree using iTol Software](#5.4 Visualize tree using iTol Software)
...
#### 1.1 Download coral protein sequences
...
#### 1.2 Download NOX reference sequences
...
#### 1.3 Download transcriptomic sequences and translate to proteins
...
#### 2.1 Query annotated sequences for NOX proteins
...
#### 3.1 Create protein family (pfam) profiles using hidden markov models (hmm) for each NOX-type 
...
#### 3.2 Query unannotated coral protein sequences using hmmSearch
...
#### 3.3 Identify NOX-like coral sequences
...
#### 3.4 Verify identified NOX-like sequences against *nr* database
...
#### 4.1 Download NOX-human domains from pfam database
...
#### 4.2 Identify specific domains in NOX-like sequences

Specific domains in coral NOX-like sequences were identified using the `.seed` files and `hmmSearch`, similar to that described in [Section 3.1][#3.1 Create protein family (pfam) profiles using hidden markov models (hmm) for each NOX-type ], [Section 3.2][#3.2 Query unannotated coral protein sequences using hmmSearch], and [Section 3.3][3.3 Identify NOX-like coral sequences]. The only exception to the pipeline described in Sections 3.1 - 3.3 is in the creation of the HMM profile. Since a `.seed` file was downloaded for the known NOX domains, the following script was used in `bash` to create a HMM profile:
...
#### 5.3 Overall bash script to execute for creating a tree
...
- `biopythonSeqFasta` from [Section 3.3](# 3.3 Identify NOX-like coral sequences)
- `MSAScript` from [Section 5.1](#5.1 Align all coral NOX-like sequences)
- `biopythonConvertScript` in [Section 5.2](#5.2 Create a tree file)
- `biopythonCreateTreeScript` in [Section 5.2](#5.2 Create a tree file)
...
#### 5.4 Visualize tree using iTol Software
